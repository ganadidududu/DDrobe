...
2. summary는 추천 사이즈와 fit score를 포함하고, 전체 실루엣과 핵심 장단점을 최소 4문장으로 설명해라.
...
5. 각 부위는 기준 수치, 상품 수치, cm 차이와 방향을 명시하고, 그 차이가 실루엣과 움직임 여유에 어떤 의미가 있는지 최소 3문장으로 설명해라.
6. 값이 유사 범위여도 차이가 0이 아니라면 작아지는 방향인지 커지는 방향인지 문장에 반영해라.
7. recommendationReason은 추천 사이즈와 다른 모든 후보 점수를 비교하고, 가장 가까운 대안과의 차이, 부위별 장단점의 균형, 최종 추천 이유를 최소 6문장으로 설명해라.
